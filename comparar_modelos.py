...
CANDIDATE_PATHS = [
    Path("reports/candidate_v2/metricas_modelo.json"),
    *Path("reports/metrics/candidates").glob("*/metrics.json"),
]
...
def compare_models() -> dict:
    active = load_metrics(ACTIVE_PATH)
    candidates = [
        load_metrics(path) for path in CANDIDATE_PATHS if path.exists()
    ]
